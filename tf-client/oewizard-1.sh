...
db1=$(aws rds describe-db-instances --region eu-west-2 --db-instance-identifier dwp-demo-ha2-az2 --query "DBInstances[].Endpoint.Address" | jq -r .[])
if [ $db1 != "" ];then 
    nmap $db1 -Pn -p 1521 | grep open
    echo "swingbench oewizard"
    date
...
db1=$(aws rds describe-db-instances --region eu-west-2 --db-instance-identifier dwp-demo-single --query "DBInstances[].Endpoint.Address" | jq -r .[])
if [ $db1 != "" ];then 
    nmap $db1 -Pn -p 1521 | grep open
    echo "swingbench oewizard"
    date
